            ...
            try:
                ...
                for q_data in questions:
                    if q_data['question'] and len(q_data['options']) == 4:
                        # Create new question
                        new_question = Questions(chapter_id=int(chapter_id),
                        ...
                        for option_text in q_data['options']:
                            new_option = Options(question_id=new_question.qid,
                                                 option=option_text)
                            db.session.add(new_option)
                        db.session.commit()

                        # Set correct answer
                        correct_option_text = q_data['options'][
                            q_data['correct_option'] - 1]
                        result = db.session.execute(
                            text(
                                "SELECT option_id FROM options WHERE option = :option AND question_id = :question_id"
                            ), {
                                "option": correct_option_text,
                                "question_id": new_question.qid
                            })
                ...
                if added_count > 0:
                    flash(
                        f'Successfully added {added_count} questions from the file!',
                        'success')
                else:
                    flash(
